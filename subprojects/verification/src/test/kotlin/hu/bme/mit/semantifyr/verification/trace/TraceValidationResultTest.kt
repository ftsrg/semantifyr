...
class TraceValidationResultTest {
    ...
    fun `Passed maps to Valid`() {
        val mapped = TraceValidationResult.from(result(VerificationVerdict.Passed))
        assertThat(mapped).isInstanceOf(TraceValidationResult.Valid::class.java)
    ...
    fun `Failed maps to Invalid`() {
        val mapped = TraceValidationResult.from(result(VerificationVerdict.Failed))
        assertThat(mapped).isInstanceOf(TraceValidationResult.Invalid::class.java)
    ...
    fun `Inconclusive maps to Inconclusive`() {
        val mapped = TraceValidationResult.from(result(VerificationVerdict.Inconclusive))
        assertThat(mapped).isInstanceOf(TraceValidationResult.Inconclusive::class.java)
        assertThat(mapped.isValid).isFalse
        assertThat(mapped.isInvalid).isFalse
    }
    ...
    fun `Errored maps to Errored`() {
        val mapped = TraceValidationResult.from(result(VerificationVerdict.Errored))
        assertThat(mapped).isInstanceOf(TraceValidationResult.Errored::class.java)
        assertThat(mapped.isValid).isFalse
        assertThat(mapped.isInvalid).isFalse
    }
    ...
    fun `from preserves the underlying verification result`() {
        ...
        val mapped = TraceValidationResult.from(verification)
