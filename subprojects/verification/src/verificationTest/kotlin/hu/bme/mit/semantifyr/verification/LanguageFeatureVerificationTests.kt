...
class LanguageFeatureVerificationTests {
    ...
    @ParameterizedTest
    @MethodSource
    suspend fun `Control flow`(verificationCase: VerificationCase) {
        helper.checkVerificationCase(controlFlowModel, verificationCase, Portfolios.ThetaFull)
    }

    @ParameterizedTest
    @MethodSource
    suspend fun `Classes and inheritance`(verificationCase: VerificationCase) {
        helper.checkVerificationCase(classesModel, verificationCase, Portfolios.ThetaFull)
    }

    @ParameterizedTest
    @MethodSource
    suspend fun `Inline calls`(verificationCase: VerificationCase) {
        helper.checkVerificationCase(inlineCallsModel, verificationCase, Portfolios.ThetaFull)
    }

    @ParameterizedTest
    @MethodSource
    suspend fun `Property calls`(verificationCase: VerificationCase) {
        helper.checkVerificationCase(propertyCallsModel, verificationCase, Portfolios.ThetaFull)
    }

    @ParameterizedTest
    @MethodSource
    suspend fun `Variable dispatch`(verificationCase: VerificationCase) {
        helper.checkVerificationCase(variableDispatchModel, verificationCase, Portfolios.ThetaFull)
    }

    @ParameterizedTest
    @MethodSource
    suspend fun `Enums`(verificationCase: VerificationCase) {
        helper.checkVerificationCase(enumsModel, verificationCase, Portfolios.ThetaFull)
    }

    @ParameterizedTest
    @MethodSource
    suspend fun `Parametric calls`(verificationCase: VerificationCase) {
        helper.checkVerificationCase(parametricModel, verificationCase, Portfolios.ThetaFull)
    }

    @ParameterizedTest
    @MethodSource
    suspend fun `Temporal operators`(verificationCase: VerificationCase) {
        helper.checkVerificationCase(temporalModel, verificationCase, Portfolios.ThetaFull)
    }

    @ParameterizedTest
    @MethodSource
    suspend fun `Expressions`(verificationCase: VerificationCase) {
        helper.checkVerificationCase(expressionsModel, verificationCase, Portfolios.ThetaFull)
    }

    @ParameterizedTest
    @MethodSource
    suspend fun `Local variables`(verificationCase: VerificationCase) {
        helper.checkVerificationCase(localVarsModel, verificationCase, Portfolios.ThetaFull)
    }

    @ParameterizedTest
    @MethodSource
    suspend fun `Advanced features`(verificationCase: VerificationCase) {
        helper.checkVerificationCase(advancedFeaturesModel, verificationCase, Portfolios.ThetaFull)
    }

    @ParameterizedTest
    @MethodSource
    suspend fun `Navigation edge cases`(verificationCase: VerificationCase) {
        helper.checkVerificationCase(navigationModel, verificationCase, Portfolios.ThetaFull)
    }
}
