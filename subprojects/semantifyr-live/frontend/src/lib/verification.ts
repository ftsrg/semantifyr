...
export interface VerificationCaseState {
  ...
  /** End-state of the witness cross-validation (post-verify). */
  ...
  portfolioId?: string
  /** Stage breakdown for the witness cross-validation, parallel to {@link metrics}. */
  validationMetrics?: VerificationMetrics
  /** Engine that produced the cross-validation verdict. */
  validationBackendId?: string
  /** Portfolio id used for the cross-validation. */
 ...
 * panel's Cancel button reaches both flows uniformly. The handle's cancel both fires
 * `$/cancelRequest` (so the LSP can abort the running verifier without waiting for the progress
 * poll) AND resets the case to stale so the UI doesn't pretend it's still running.
 */
...
export async function validateWitness(
  ...
  try {
    ...
    const response =
      token === undefined
        ? ((await client.sendRequest('workspace/executeCommand', params)) as WitnessValidationResultResponse | null)
        : ((await client.sendRequest('workspace/executeCommand', params, token)) as WitnessValidationResultResponse | null)
