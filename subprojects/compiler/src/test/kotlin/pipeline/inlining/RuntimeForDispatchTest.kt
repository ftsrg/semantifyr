...
class RuntimeForDispatchTest : PackageExpanderTestBase() {
    ...
    fun `runtime for over a containment range dispatches the inline call across each candidate`() {
        ...
        // The runtime for survives (loop variable still present), but the
        // `inline` inside the body must have been expanded.
