...
/**
 * Expression-level (property) inlining tests against full OXSTS packages.
 * Mirrors [PackageInliningOperationTest]: self-calls, navigation calls,
 * polymorphism / redefinition, multi-level navigation, and multi-level
 * inheritance - but against property expressions instead of inline
 * transition operations.
 *
 * "Inlining succeeded" for expressions means the property expression
 * contains no residual [CallSuffixExpression] to a property declaration.
 */
class PackageInliningExpressionTest : PackageExpanderTestBase() {
    ...
    @org.junit.jupiter.api.Disabled(
        "Expression-level variable dispatching requires if-expression " +
            "support in the language; the expander currently rejects it " +
            "with a source-annotated error."
    )
    @Test
    fun `variable dispatch in a property expression is rejected`() {
        // Intentionally placeholder: once if-expressions exist, this test
        // should assert that the property body expands to an if-expression
        // chain over the variable's candidate instances.
    }
    ...
    private fun assertPropertyContains(prepared: Prepared, substring: String) {
        val propertyText = serializeNormalized(prepared.inlinedOxsts.property)
